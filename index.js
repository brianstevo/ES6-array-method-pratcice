...
const d = inventors.reduce((acc, el) => {
    return acc + (el.passed - el.year);
}, 0)
console.log(4, d);
//specify acc as zero while using reduce


// 5. Sort the inventors by years lived
const e = inventors.sort((a, b) => {
...
const de = links.map(el => {
    return el.textContent;
}).filter(x => x.includes('de'));
// 7. sort Exercise
// Sort the people alphabetically by last name
const f = inventors.sort((a, b) => {
    const first = a.last;
    const second = b.last;
